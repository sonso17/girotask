                ...
                <div id="quadrat">
                    ...
                    <div class="container1">
                        <div class="container2"><label class="labelregistre" for="fnUsuari">Nom d'usuari</label>
                            <input class="inputregistre"  id="nomusuarimod"  type="text" name="fnUsuari" >
                        </div>
                        <div class="container2">
                            <label class="labelregistre" for="lnUsuari">Cognom d'usuari</label>
                            <input class="inputregistre" id="cognomusuarimod"  type="text" name="lnUsuari">
                        </div>
                    </div>
                    <div class="container1">
                        <div class="container2"><label class="labelregistre" for="emailusuari">Correu de
                                l'usuari</label>
                            <input class="inputregistre" id="correuusuarimod"  type="text" name="emailUsuari">
                        </div>
                        <div class="container2">
                            <label class="labelregistre" for="passwd">Contrassenya</label>
                            <input class="inputregistre" id="passwdusuarimod"  type="text" name="passwd">
                        </div>
                    </div>
                    <div style="display: flex; flex-direction: column; align-items: center;">
                        ...
                        <br>
                        <div>
                            <button class="botochulo" id="botoactualitzarusuari" @click="ActualitzarUsuari()" type="submit">Actualitzar </button>
                            <button class="botochulo" id="botoeliminarusuari" @click="EliminarUsuari()" type="submit">Eliminar </button>
                        </div>
                    </div>
                </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        let idusuariurl = params.substring(18, 21)
            axios.get("http://localhost/API/"+ sessionStorage.tokenUsuari +"/selectOneUser/"+idusuariurl)
                .then(resultat => {
                    this.user = resultat.data
                    document.getElementById("nomusuarimod").value = this.user[0].Nom
                    document.getElementById("cognomusuarimod").value = this.user[0].Cognom
                    document.getElementById("correuusuarimod").value = this.user[0].Email
                    document.getElementById("passwdusuarimod").value = this.user[0].Contrassenya
        ...
        axios.post('http://localhost/API/' + sessionStorage.tokenUsuari + '/modificarUsuari/' + iduserurl,
                { data: { 
